...
class ldap(connection):
    ...
    def kerberos_login(self, domain, username, password="", ntlm_hash="", aesKey="", kdcHost="", useCache=False):
        ...
        self.username = username if not self.args.use_kcache else self.username
    ...
    def bloodhound(self):
        ...
        if self.args.kerberos:
            self.logger.highlight(
                "Using kerberos auth without ccache, getting TGT")
            auth.get_tgt()
        ...
        try:
            ...
            self.logger.debug(
                f"BloodHound collection failed: {e.__class__.__name__} - {e}", exc_info=True)
            return
