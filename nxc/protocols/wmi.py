...
class wmi(connection):
    ...
    def check_if_admin(self):
        try:
            dcom = DCOMConnection(self.remoteName, self.username, self.password, self.domain, self.lmhash, self.nthash,
                                  oxidResolver=True, doKerberos=self.doKerberos, kdcHost=self.kdcHost, aesKey=self.aesKey, remoteHost=self.host)
    ...
    def execute(self, command=None, get_output=False, use_powershell=False):
        ...
        if self.args.exec_method == "wmiexec":
            exec_method = wmiexec.WMIEXEC(self.remoteName, self.username, self.password, self.domain, self.lmhash, self.nthash,
                                          self.doKerberos, self.kdcHost, self.host, self.aesKey, self.logger, self.args.exec_timeout, self.args.codec)
            output = exec_method.execute(
                command, get_output, use_powershell=use_powershell)

        elif self.args.exec_method == "wmiexec-event":
            exec_method = wmiexec_event.WMIEXEC_EVENT(self.remoteName, self.username, self.password, self.domain, self.lmhash, self.nthash,
                                                      self.doKerberos, self.kdcHost, self.host, self.aesKey, self.logger, self.args.exec_timeout, self.args.codec)
            output = exec_method.execute(
                command, get_output, use_powershell=use_powershell)
